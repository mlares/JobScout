...
against the resume, with CV and cover-letter PDF downloads. The curriculum
repository's evidence-backed CV-family selector/tailor consumes the job
description already attached to the selected search result.

Generated downloads are served by a small capability-token API on :8000; the
main user experience remains entirely in the Gradio wizard.
"""
...
from job_scout.artifact_store import ArtifactAccessError, register_files
from job_scout.cv_generation import CV_FAMILIES, CVGenerationError, CVGenerationRequest, CVGenerationResult, generate_cv
from job_scout.graph import checkpoint_values
from job_scout.graph.schemas import JobPosting, Profile, RankedJob
from job_scout.profile import extract_profile
from job_scout.renderer import render_application_pdfs
from job_scout.role_targets import DEFAULT_TARGET_ROLE, ROLE_OPTIONS, normalize_target_role
...
.js-section-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
.js-section-head .js-section-label { margin: 0; }
.js-pdf-link { display: inline-flex; align-items: center; padding: 8px 13px; border-radius: 8px;
  background: var(--primary-500); color: white !important; font-size: 0.82rem; font-weight: 600;
  text-decoration: none !important; }
.js-pdf-link:hover { background: var(--primary-600); }
.js-generated-files { font-size: 0.82rem; margin: -4px 0 0; }
.js-generated-files a { margin-right: 12px; }
...
def _results_html(result: RunResult) -> str:
    ...
    if not result.ranked_jobs:
        return (
            ...
            "<div>No eligible matches were found in the available job sources for this role. "
            "Try a nearby role, enable another job source, or check back later.</div></div>"
        )
...
def _results_page(
    ...
    page: int | float,
    source: str,
) -> tuple[str, dict, str, int, dict, dict]:
    """Render one source-filtered page without rerunning search or ranking."""
    if result is None:
        hidden = gr.update(visible=False)
        return "", hidden, "", 1, hidden, hidden
    jobs = result.ranked_jobs
    if source and source != ALL_SOURCES:
        jobs = [ranked for ranked in jobs if ranked.job.source == source]
    total = len(jobs)
    pages = max(1, (total + JOBS_PER_PAGE - 1) // JOBS_PER_PAGE)
    current = min(max(int(page or 1), 1), pages)
    start = (current - 1) * JOBS_PER_PAGE
    shown = jobs[start : start + JOBS_PER_PAGE]
    if shown:
        html = _results_html(RunResult(ranked_jobs=shown))
    elif source and source != ALL_SOURCES:
        html = (
            '<div class="js-empty"><div class="js-empty-icon">🔍</div>'
            f"<div>No ranked jobs from {escape(source)} in this search.</div></div>"
        )
    else:
        html = _results_html(result)
    choices = [(f"{r.job.title} — {r.job.company} (fit {r.fit_score})", r.job.job_id) for r in shown]
    select = gr.update(choices=choices, value=None, visible=bool(choices))
    label = f"Page {current} of {pages} · {total} job{'s' if total != 1 else ''}"
    return (
        html,
        select,
        label,
        current,
        gr.update(interactive=current > 1, visible=total > JOBS_PER_PAGE),
        gr.update(interactive=current < pages, visible=total > JOBS_PER_PAGE),
    )


def on_results_page(result: RunResult | None, page: int | float, source: str, delta: int = 0):
    """Change page or source filter using the already-ranked in-memory result."""
    target = 1 if delta == 0 else int(page or 1) + delta
    return _results_page(result, target, source)
...
def _pack_html(
    result: TailorResult,
    cv_generation: CVGenerationResult | None = None,
    cv_generation_error: str = "",
    ...
    cover_download = (
        f'<a class="js-pdf-link" href="{escape(cover_letter_url)}" download>Download cover letter in PDF</a>'
        if cover_letter_url
        else ""
    )
    cover_warning = (
        f'<div class="js-fab-warn">Cover-letter PDF unavailable: {escape(cover_letter_error)}</div>'
        if cover_letter_error
        else ""
    )
    if cv_generation and cv_generation.pdf_filename:
        scores = ", ".join(f"{item.family}: {item.score:.1f}" for item in cv_generation.rankings)
        cv_download = (
            f'<a class="js-pdf-link" href="{escape(cv_generation.download_url("pdf"))}" download>'
            "Download tailored CV in PDF</a>"
        )
        cv_files = (
            '<p class="js-generated-files">'
            f'CV family: <code>{escape(cv_generation.selected_family)}</code> · {escape(cv_generation.mode)} · '
            f'<a href="{escape(cv_generation.download_url("tex"))}" download>LaTeX source</a>'
            f'<a href="{escape(cv_generation.download_url("notes"))}" download>Tailoring notes</a><br>'
            f'<span class="js-muted">Match scores: {escape(scores or "unavailable")}</span></p>'
        )
    elif cv_generation_error:
        cv_download = ""
        cv_files = f'<div class="js-fab-warn">CV PDF unavailable: {escape(cv_generation_error)}</div>'
    else:
        cv_download = ""
        cv_files = ""
    return (
        ...
        f'<div class="js-section-head"><p class="js-section-label">Cover letter</p>{cover_download}</div>'
        f"{cover_warning}"
        f'<div class="js-card js-letter">{escape(pack.cover_letter)}</div>'
        f'<div class="js-section-head"><p class="js-section-label">Tailored CV</p>{cv_download}</div>'
        f"{cv_files}"
...
REMOTE_CHOICE = "Remote — accepts candidates residing in Argentina"


def _selection_to_prefs(selection: list[str], target_role: str | None = None) -> dict:
    """The chooser's ticked boxes as a preferences dict."""
    return {
        "locations": [s for s in selection if s != REMOTE_CHOICE],
        "remote": REMOTE_CHOICE in selection,
...
def _preference_role(preferences: dict | None) -> str:
    """Restore the chosen target role, with a default for older saved profiles."""
    return normalize_target_role((preferences or {}).get("target_role"))


def on_prefs_change(
    selection: list[str], target_role: str, profile: Profile | None, cv_text: str, thread_id: str
) -> None:
    """Persist the location and role chooser state."""
    if profile is None:
        return
    prefs = _selection_to_prefs(selection, target_role)
    _ = thread_id
    candidate_store.save_candidate(profile, cv_text, prefs)


def on_add_location(new_location: str, choices: list[str], selection: list[str], target_role, profile, cv_text, thread_id):
    ...
    on_prefs_change(selection, target_role, profile, cv_text, thread_id)  # programmatic updates don't fire .change
    return choices, gr.update(choices=choices, value=selection), ""


def _on_load(thread_id: str):
    """Restore a saved candidate without rerunning profile extraction.
    ...
    profile_state, loc_choices_state, loc_group, target_role).
    """
    _ = thread_id
    ...
    target_role = _preference_role(stored.preferences)
    note = (
...
def on_upload(file_path: str | None, thread_id: str):
    ...
    Outputs: (page_start, page_profile, profile_html, cv_text_state,
    start_status, profile_state, loc_choices_state, loc_group, target_role).
    """
    ...
    candidate_store.save_candidate(profile, cv_text, _selection_to_prefs(selected, DEFAULT_TARGET_ROLE))
    yield (
        *go,
        ...
        choices,
        gr.update(choices=choices, value=selected),
        gr.update(value=DEFAULT_TARGET_ROLE),
    )
...
def on_find(
    ...
    Outputs include the two pages, rendered jobs, footer, selection controls,
    pagination state, and source filter.
    """
    go = gr.update(visible=False), gr.update(visible=True)
    if profile is None:
        yield (
    ...
    target_role = normalize_target_role(target_role)
    prefs = _selection_to_prefs(loc_selection or [], target_role)
    effective = candidate_store.effective_profile(profile, prefs)
    candidate_store.save_candidate(profile, cv_text, prefs)

    trailing = (gr.update(), gr.update(), "", 1, gr.update(), gr.update())
    yield (*go, _loading_html("Searching for jobs…"), "", gr.update(), *trailing)
    result = RunResult()
    for kind, payload in stream_search(
        ...
        if kind == "status":
            yield (*go, _loading_html(str(payload)), "", gr.update(), *trailing)
        elif kind == "result":
            result = payload  # type: ignore[assignment]

    html, select, page_label, page, previous, next_page = _results_page(result, 1, ALL_SOURCES)
    sources = sorted({ranked.job.source for ranked in result.ranked_jobs})
    source_update = gr.update(
        choices=[ALL_SOURCES, *sources],
        value=ALL_SOURCES,
        visible=bool(sources),
    )
    yield (*go, html, _footer_html(result), select, result, source_update, page_label, page, previous, next_page)


def on_zip(zip_path: str | None) -> str | None:
    """Store the optional LinkedIn export path in Gradio session state."""
    return zip_path


def _retrieved_job(thread_id: str, selected_job_id: str) -> JobPosting | None:
    """Find the selected posting in the server-side search checkpoint."""
    values = checkpoint_values(thread_id)
    for ranked in values.get("ranked_jobs") or []:
        if isinstance(ranked, RankedJob) and ranked.job.job_id == selected_job_id:
            return ranked.job
    return None


CV_CLOSEST_MATCH = "Find the closest CV match"
CV_GENERATE_NEW = "Generate a new CV from the job description"


def _cover_letter_download(pack, profile: Profile | None) -> tuple[str, str]:
    """Render and register the cover-letter PDF for this application request."""
    name = (profile.name if profile else None) or "Candidate"
    location = profile.locations[0] if profile and profile.locations else "Argentina"
    rendered = render_application_pdfs(
        pack,
        name,
        Path(tempfile.mkdtemp(prefix="job_scout_application_")),
        location=location,
    )
    if rendered.cover_letter.pdf_path is None:
        return "", rendered.message or "PDF compilation failed."
    try:
        return register_files({"cover-letter": rendered.cover_letter.pdf_path}).url("cover-letter"), ""
...
def on_tailor(
    ...
    thread_id: str,
    linkedin_zip: str | None,
    profile: Profile | None,
    cv_option: str = CV_CLOSEST_MATCH,
    base_family: str = "Automatic",
    model: str = "",
):
    """Step 3 → 4: tailor an application for the selected job on the SAME thread.

    The graph and curriculum adapter both resolve the original job from the
    server-side checkpoint; no description is accepted from the browser here.
    ...
    job = _retrieved_job(thread_id, selected_job_id)
    if job is None:
        yield (*stay, _status("The selected job is no longer available in this search session.", error=True), "", hidden, hidden)
        return
    ...
    if result.pack is not None:
        cover_letter_url, cover_letter_error = _cover_letter_download(result.pack, profile)
    cv_generation = None
    cv_generation_error = ""
    if result.pack is not None:
        modes = {CV_CLOSEST_MATCH: "offline", CV_GENERATE_NEW: "ai"}
        mode = modes.get(cv_option)
        if mode is None:
            cv_generation_error = "Unknown CV preparation option."
        else:
            action = "Finding the closest CV family" if mode == "offline" else "Generating a new evidence-grounded CV"
            yield (*go, _loading_html(f"{action} from the selected job description…"), "", hidden, hidden)
            try:
                cv_generation = generate_cv(
                    CVGenerationRequest(
                        job_description=job.description,
                        company=job.company,
                        role=job.title,
                        mode=mode,
                        base_family=None if base_family == "Automatic" else base_family,
                        model=(model or None) if mode == "ai" else None,
                        build_pdf=True,
                    )
                )
            except CVGenerationError as exc:
                cv_generation_error = str(exc)
    yield (
        *go,
        _pack_html(result, cv_generation, cv_generation_error, cover_letter_url, cover_letter_error),
        _tailor_footer_html(result),
        hidden,
        hidden,
    )
...
def reset():
    ...
    loc_choices_state, loc_group, target_role, results_state, source_filter,
    page_info, results_page_state, previous_page, next_page).
    """
...
def build_app() -> gr.Blocks:
    ...
    with gr.Blocks(title="Job Scout") as demo:
        ...
        with gr.Group(visible=False, elem_classes=["js-page"]) as page_profile:
            ...
            profile_out = gr.HTML()
            gr.HTML('<p class="js-section-label" style="margin-top:14px">Which role should we target?</p>')
            target_role = gr.Dropdown(
                label="",
                show_label=False,
                choices=list(ROLE_OPTIONS),
        ...
        with gr.Group(visible=False, elem_classes=["js-page"]) as page_results:
            ...
            source_filter = gr.Dropdown(label="Filter by listing source", choices=[], visible=False, interactive=True)
            results_out = gr.HTML()
            with gr.Row():
                previous_page = gr.Button("Previous", size="sm", visible=False)
                page_info = gr.HTML()
                next_page = gr.Button("Next", size="sm", visible=False)
            footer_out = gr.HTML()
            ...
            cv_option = gr.Radio(
                ...
                choices=[CV_CLOSEST_MATCH, CV_GENERATE_NEW],
                value=CV_CLOSEST_MATCH,
                interactive=True,
            )
            with gr.Accordion("CV options", open=False):
                cv_base_family = gr.Dropdown(
                    label="Base CV family",
                    choices=["Automatic", *CV_FAMILIES],
                    value="Automatic",
                    interactive=True,
                )
                cv_model = gr.Textbox(
                    label="OpenAI model (used only when generating a new CV)",
                    placeholder="Uses OPENAI_MODEL when blank",
                )
                gr.HTML(
                    '<p class="js-muted">The closest-match option uses curriculum’s deterministic selector without an '
                    "additional CV-generation API call. Generating a new CV sends the selected posting, base CV, shared "
                    "experience, and evidence registers to OpenAI. Job Scout still uses its configured LLM for the cover "
                    "letter and on-screen tailored preview.</p>"
                )
        ...
        with gr.Group(visible=False, elem_classes=["js-page"]) as page_tailor:
            ...
            tailor_out = gr.HTML()
            # Kept as hidden outputs for the callback tuple contract;
            # visible PDF actions now live beside their content section.
            pdf_btn = gr.DownloadButton("CV PDF", visible=False)
            tex_btn = gr.DownloadButton("Cover-letter PDF", visible=False)
        ...
        loc_group.input(on_prefs_change, inputs=[loc_group, target_role, profile_state, cv_text_state, thread_id])
        target_role.input(on_prefs_change, inputs=[loc_group, target_role, profile_state, cv_text_state, thread_id])
        loc_add.click(
            on_add_location,
            inputs=[loc_new, loc_choices_state, loc_group, target_role, profile_state, cv_text_state, thread_id],
        ...
        find_btn.click(
            on_find,
            inputs=[cv_text_state, profile_state, thread_id, loc_group, target_role, max_jobs],
            outputs=[
                page_profile,
                page_results,
                results_out,
        ...
        source_filter.change(
            on_results_page,
            inputs=[results_state, results_page_state, source_filter],
            outputs=[results_out, job_select, page_info, results_page_state, previous_page, next_page],
        )
        previous_page.click(
            lambda result, page, source: on_results_page(result, page, source, -1),
            inputs=[results_state, results_page_state, source_filter],
            outputs=[results_out, job_select, page_info, results_page_state, previous_page, next_page],
        )
        next_page.click(
        ...
        tailor_btn.click(
            on_tailor,
            inputs=[job_select, thread_id, linkedin_zip_state, profile_state, cv_option, cv_base_family, cv_model],
        ...
        # Restore on page load without re-running profile extraction.
        demo.load(
            _on_load,
            inputs=[thread_id],
            outputs=[
...
def main() -> None:
    """Serve the wizard and its capability-scoped download endpoint."""
    from job_scout.api import DOWNLOAD_PORT, serve_in_thread

    serve_in_thread()
    print(f"Download API: http://localhost:{DOWNLOAD_PORT}")
