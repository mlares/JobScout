...
``{"locations": [...], "remote": bool, "target_role": str}``. It lives NEXT TO the profile rather
...
_STORE_DIR = Path(os.getenv("JOB_SCOUT_DATA_DIR", Path(__file__).resolve().parents[2] / "data")) / "candidate"
...
class StoredCandidate(NamedTuple):
    ...
    profile: Profile
    cv_text: str
    preferences: dict | None  # locations, remote, target_role; None on v1 files
...
def save_candidate(profile: Profile, cv_text: str, preferences: dict | None = None) -> None:
    ...
    payload = {"version": 3, "profile": profile.model_dump(), "cv_text": cv_text, "preferences": preferences}
