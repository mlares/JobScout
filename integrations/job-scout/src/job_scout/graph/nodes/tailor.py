...
from job_scout.role_targets import normalize_target_role, role_guidance
...
def tailor(state: AgentState) -> dict:
    ...
    target_role = normalize_target_role(selected_role) if selected_role else "best role supported by the CV and target job"
    prompt = TAILOR_PROMPT.format(
        research_rule=RESEARCH_RULE if research else "",
        target_role=target_role,
        role_guidance=role_guidance(selected_role),
        profile=_render_profile(profile, selected_role),
