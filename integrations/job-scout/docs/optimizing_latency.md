...
The complaint that started it: an interactive job search felt endless —
the user starts a search and waits the better part of a minute. The wrong
...
3. In Opik, open both traces side by side: compare the `fetch_jobs` and
   ...
   record which entry point triggered it (`ui`, batch).
