...
def test_search_passes_profile_and_nulls_selected_job_id(monkeypatch, sample_profile):
    ...
    assert fake.captured_inputs["jobs"] == [] and fake.captured_inputs["ranked_jobs"] == []
...
def test_stream_search_yields_result(monkeypatch, sample_profile):
    ...
    events = list(stream_search(sample_profile, thread_id="t1", tags=["ui"], max_jobs=20))
    assert events[-1][0] == "result"
    result = events[-1][1]
    assert result.jobs_sources == ["cache"]
    assert result.failed is False
    assert fake.captured_inputs["search_limit"] == 20
