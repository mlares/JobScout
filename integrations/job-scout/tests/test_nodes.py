...
def test_fetch_jobs_uses_request_limit_and_caps_it(monkeypatch, sample_profile, sample_jobs):
    limits = []
    ...
    monkeypatch.setattr(fetch_mod, "run_search", fake_run_search)
    monkeypatch.setattr(fetch_mod, "get_chat_model", lambda *a, **k: tool_calling_llm([{"args": {"query": "ds"}}]))
    fetch_jobs({"profile": sample_profile, "llm_calls": 0, "search_limit": 20})
    fetch_jobs({"profile": sample_profile, "llm_calls": 0, "search_limit": 999})
    assert limits == [20, fetch_mod.MERGED_CEILING]
...
def test_selected_role_is_exact_and_skips_fetch_llm(monkeypatch, sample_profile, sample_jobs):
    seen = {}
    monkeypatch.setattr(fetch_mod, "get_chat_model", lambda *a, **k: (_ for _ in ()).throw(AssertionError("no LLM")))
    ...
    monkeypatch.setattr(fetch_mod, "run_search", fake_run_search)
    out = fetch_jobs(
        {"profile": sample_profile, "target_role": "applied ai lead", "llm_calls": 2, "reformulation_count": 0}
    )
    assert seen["query"] == "applied ai lead"
    assert out["target_role"] == "applied ai lead"
    assert out["llm_calls"] == 2


def test_argentina_search_uses_local_and_compatible_remote_lanes(monkeypatch, sample_profile):
    profile = sample_profile.model_copy(update={"locations": ["Córdoba, Argentina"], "remote_ok": True})
    calls = []
    local = make_job("local", "AI Lead", "AR Co", "jsearch").model_copy(update={"location": "Buenos Aires, Argentina"})
    worldwide = make_job("world", "AI Lead", "Global Co", "remotive", True).model_copy(update={"location": "Worldwide"})
    us_only = make_job("us", "AI Lead", "US Co", "remotive", True).model_copy(update={"location": "US only"})

    def fake_run_search(**kwargs):
        calls.append(kwargs)
        return ([local], ["jsearch"]) if not kwargs["remote"] else ([worldwide, us_only], ["remotive"])

    monkeypatch.setattr(fetch_mod, "run_search", fake_run_search)
    out = fetch_jobs(
        {"profile": profile, "target_role": "ai technical lead", "residence_country": "Argentina", "llm_calls": 0}
    )

    assert calls[0]["location"] == "Argentina" and calls[0]["country"] == "ar" and calls[0]["remote"] is False
    assert calls[0]["include_remote_source"] is False
    assert calls[1]["location"] is None and calls[1]["country"] == "ar" and calls[1]["remote"] is True
    assert [job.job_id for job in out["jobs"]] == ["local", "world"]


def test_argentina_unknown_remote_score_is_capped(monkeypatch, sample_profile):
    profile = sample_profile.model_copy(update={"locations": ["Argentina"], "remote_ok": True})
    job = make_job("j1", "AI Lead", "Co", "remotive", True).model_copy(
        update={"location": "Remote", "location_eligibility": "unknown"}
    )

    def fake_model(*args, **kwargs):
        return structured_llm(
            JobScores(scores=[JobScore(job_id="j1", fit_score=92, fit_explanation="Strong technical match")])
        )
    ...
    out = rank_jobs(
        {
            "profile": profile,
            "target_role": "applied ai lead",
            "residence_country": "Argentina",
            "jobs": [job],
            "llm_calls": 0,
        }
    )
    assert out["ranked_jobs"][0].fit_score == 55
    assert "Argentina" in out["ranked_jobs"][0].gaps[0]


def test_zero_score_is_not_a_recommendation(monkeypatch, sample_profile):
    jobs = [make_job("reject", "Chef", "No Fit"), make_job("match", "Data Scientist", "Fit Co")]

    def fake_model(*args, **kwargs):
        return structured_llm(
            JobScores(
                scores=[
                    JobScore(job_id="reject", fit_score=0, fit_explanation="Unrelated role"),
                    JobScore(job_id="match", fit_score=72, fit_explanation="Relevant role"),
                ]
            )
        )
    ...
    monkeypatch.setattr(rank_mod, "get_chat_model", fake_model)
    out = rank_jobs({"profile": sample_profile, "jobs": jobs, "llm_calls": 0})
    assert [ranked.job.job_id for ranked in out["ranked_jobs"]] == ["match"]
