...
from job_scout.role_targets import DEFAULT_TARGET_ROLE
...
def test_on_load_restores_candidate_and_opens_step_two(tmp_store, sample_profile):
    ...
    page_start, page_profile, profile_html, cv_text, profile, loc_choices, loc_group, target_role = app_module._on_load("t1")
    ...
    assert target_role["value"] == DEFAULT_TARGET_ROLE
...
def test_on_find_searches_the_chosen_locations(tmp_store, sample_profile, monkeypatch):
    ...
    def fake_stream(profile, **kwargs):
        ...
        seen["target_role"] = kwargs.get("target_role")
        seen["residence_country"] = kwargs.get("residence_country")
        seen["max_jobs"] = kwargs.get("max_jobs")
    ...
    list(app_module.on_find("cv text", sample_profile, "t1", selection, "applied ai lead"))

    assert seen["profile"].locations == ["Tokyo, Japan"] and seen["profile"].remote_ok is True
    assert seen["target_role"] == "applied ai lead"
    assert seen["residence_country"] == "Argentina"
    assert seen["max_jobs"] == 20
    assert sample_profile.locations == ["Berlin, Germany"]  # the raw profile is never mutated
    assert tmp_store.load_candidate().preferences == {
...
def test_on_add_location_ticks_and_persists(tmp_store, sample_profile):
    ...
    new_choices, group_update, cleared = app_module.on_add_location(
        "  Lisbon  ", choices, ["Berlin, Germany"], "ai technical lead", sample_profile, "cv", "t1"
    )
    assert new_choices == ["Berlin, Germany", "Lisbon", app_module.REMOTE_CHOICE]
    assert group_update["value"] == ["Berlin, Germany", "Lisbon"]
    assert cleared == ""
    assert tmp_store.load_candidate().preferences == {
        "locations": ["Berlin, Germany", "Lisbon"],
        "remote": False,
        "target_role": "ai technical lead",
    }

    unchanged, no_update, _ = app_module.on_add_location(
        "Lisbon", new_choices, [], "ai technical lead", sample_profile, "cv", "t1"
    )
